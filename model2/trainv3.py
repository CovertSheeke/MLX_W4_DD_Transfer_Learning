...
class VisionLanguageTrainer:
    ...
    def setup_model(self):
        ...
        self.model = QwenModelv3(self.config.model_name)
        ...
        # Note: QwenModelv3 uses AutoModelForCausalLM, so lm_head is part of self.qwen
        for param in self.model.qwen.lm_head.parameters():
        ...
        # Note: QwenModelv3 uses AutoModelForCausalLM, so layers are in self.qwen.model.layers
        for layer in self.model.qwen.model.layers[-2:]:
        ...
        if self.model.qwen.lm_head.out_features != vocab_size:
            self.model.qwen.lm_head = nn.Linear(self.model.qwen.config.hidden_size, vocab_size, bias=False).to(self.device)
